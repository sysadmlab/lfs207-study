...
The --max-depth=1 is the same as -d 1. The **text book** explanation for -x option "**Stay on one filesystem; don’t look at directories that are not on the / partition. In this case that means ignore /dev /proc /run /sys**", because they are **not on the same filesystem ad the / partition**.  
