...
### '/var' Directory  
The **/var** directory consists of variable files - files that change frequently during system operation. The **/var/log** consists of system and application logs; the **/var/spool** consists of data that is queued from mail, print jobs, and cron jobs; the **/var/cache** consists of data cached by applications and package manager caches; and the **/var/tmp** consists of temporary files that persist across reboots. </br>  


### '/run' Directory
The **/run** directory is a **tmpfs** filesystem in other words a RAM-based temporary filesystem (or) a pseudo-filesystem that exists only when the system is powered on. The **tmpfs** filesystem is mounted at the **/run** mount point. The **/run** directory consists of runtime data that is generated during the system operation. </br>  
...
The **/run/media/(userid)/** directory has been the location where USB Flash drives have been mounted. As I have mentioned above in this thread, under the **/mnt** section, USB Flash Drives are mounted to **/run/media/userid** by the **udisks2** daemon in GUI-based systems. In Minimal installs, the SysAdmin has to manually mount the device to the **/mnt** directory.  </br>  
